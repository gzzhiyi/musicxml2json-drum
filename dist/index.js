(function (global, factory) {
	...
	(global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global["musicxml2json-drum"] = {}));
	...
	class Note {
	    ...
	    type;
	    kind;
	    constructor({ id, measureId, xmlData }) {
	        this.id = id;
	    ...
	    getBeam(noteXML) {
	        ...
	        if (!beam)
	            return null;
	        return lodash.exports.isArray(beam) ? beam.map(item => item['#text']) : [beam['#text']];
	    }
	    getCode(noteXML) {
	        return noteXML?.notations?.technical?.fret || noteXML?.notations?.technical?.root?.fret || null;
	    }
	    getData(noteXML) {
	        const code = this.getCode(noteXML);
	        if (!code) {
	            return null;
	        }
	    ...
	    getTied(noteXML) {
	        ...
	        if (!tied)
	            return null;
	        return lodash.exports.isArray(tied) ? tied[0]._type : tied._type;
	    }
	    getTimeModification(noteXML) {
	    ...
	    getTuplet(noteXML) {
	        ...
	        if (!tuplet)
	            return null;
	        return lodash.exports.isArray(tuplet) ? tuplet[0]._type : tuplet._type;
	    }
	    getType(noteXML) {
	        return noteXML.type;
	    }
	...
	class Measure {
	    ...
	    getNotes(measureXML) {
	        const notesList = [];
	        let count = 1;
	        if (lodash.exports.isEmpty(measureXML?.note)) {
	        ...
	        const notesXML = lodash.exports.isArray(measureXML.note) ? measureXML.note : [measureXML.note];
	    ...
	    isChord(noteXML) {
	        return lodash.exports.has(noteXML, 'chord');
	    }
	    calNoteDuration(note) {
	...
	class Part {
	    ...
	    constructor({ measures, partId, speed }) {
	        this.speed = speed || 1;
	    ...
	    getTimeSignature(measureXML) {
	        const timeXML = measureXML?.attributes?.time;
	        return lodash.exports.isEmpty(timeXML) ? null : {
	            beats: timeXML?.beats || 0,
	            beatType: timeXML?.['beat-type'] || 0
	        };
	    }
	    setGlobalMetronome({ beatUnit, bpm }) {
	        this.metronome = { beatUnit, bpm };
	    }
	    setGlobalTimeSignature({ beats, beatType }) {
	...
	class Parser {
	    ...
	    _speed = 1;
	    constructor(props) {
	        const { debug, instruments, speed, xmlStr } = props;
	        if (!fxp.XMLValidator.validate(xmlStr)) {
	            console.error('Invalid XML format.');
	            return;
	        }
	        globalThis.Instruments = instruments;
	    ...
	    getMeasureById(id) {
	        for (const part of this.parts) {
	            const measure = lodash.exports.find(part.measures, { id });
	            if (measure) {
	                return measure;
	            }
	        }
	    ...
	    getNoteById(id) {
	        ...
	        return lodash.exports.find(allNotes, { id }) || null;
	    }
	}

	exports.Parser = Parser;
	exports.noteTypeToNumber = noteTypeToNumber;
	exports.numberToNoteType = numberToNoteType;
